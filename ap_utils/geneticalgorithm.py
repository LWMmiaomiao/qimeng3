...
def calculate_rms(deta_s, lo_nodes,li_lo_mapping,and_nodes_sorted):
    ...
    for lo in lo_list:
        ...
        if row_indices:
            ...
            average_sum += squared_sum / len(and_nodes) 
...
def xgboost_train():
    ...
    X = np.random.rand(2000, 1) * 100  # 生成0-100之间的随机数
    y = X.ravel() + np.random.normal(0, 1e-6, 2000) # y = x + 微小噪声
...
def initial_virtual_stage_matrix(netlist,new_netlist,virtual_and_nodes_sorted,lo_nodes,po_nodes):
    ...
    for node in virtual_and_nodes_sorted:
        ...
        print("node:",node,"succ:",succ,"stage:",stage,"current_stage:",new_netlist.graph.nodes[node]["stage"])
...
def initialize_virtual_partition(netlist, and_nodes_sorted, lo_nodes, po_nodes):
    ...
    # psearch.stage_assignment_average(netlist, 3)
    psearch.stage_assignment_align_with_old(netlist,3)
    # psearch.stage_assignment_init(netlist, 3)
